...
void vision_test () {
 ...
 while (true) {
	 ...
	 if (nutral_mogii[0].y_middle_coord > 10){ //NOTE: 10 may be too small a number. Check w/ testing
		 arm_turntableA.move_relative(10, 50); //NOTE: check if this is moving in the right direction
		 arm_turntableB.move_relative(10, 50); //NOTE: check if this is moving in the right direction
	 ...
	 if (nutral_mogii[0].x_middle_coord > 10){
		 crane_rotate.move_relative(10, 50); //NOTE: check if this is moving in the right direction
		 pros::screen::print(TEXT_LARGE, 1,"arm moving left");
	 }
	 if (nutral_mogii[0].x_middle_coord < -10){
		 crane_rotate.move_relative(-10, 50); //NOTE: check if this is moving in the right direction
		 pros::screen::print(TEXT_LARGE, 1,"arm moving right");
	 } else {
